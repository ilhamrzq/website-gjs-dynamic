...
class ProductDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('product_category_name', function ($row) {
                return optional($row->category)->product_category_name ?? "";
            })
            ->filterColumn('product_category_name', function ($query, $keyword) {
                $query->whereHas('category', function ($q) use ($keyword) {
                    $q->where('product_category_name', 'ilike', "%{$keyword}%");
                });
            })
            ->orderColumn('product_category_name', function ($query, $order) {
                $query->join('products_categories', 'master_products.product_category_id', '=', 'products_categories.id')
                    ->select(['master_products.*', 'products_categories.product_category_name'])
                    ->orderBy('products_categories.product_category_name', $order);
            })

            ->addColumn('photo', function ($row) {
                $images = $row->productItemImages;
                if ($images->isEmpty()) {
                ...
                foreach ($sortedImages as $index => $image) {
                    ...
                    <div class="carousel-item ' . $activeClass . '">
                        <img class="d-block img-fluid mx-auto" src="' . $imagePath . '" alt="Product Item Image">
                    </div>';
                }
                        ...
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="' . $photoId . '_label">' . 'Product Item Image' . '</h5>
            ...
            ->addColumn('updated_by', function ($row) {
                ...
                $query->join('users as updater', 'master_products.updated_by', '=', 'updater.id')
                    ->select(['master_products.*', 'updater.name as updater_name'])
            ...
            ->rawColumns(['checkbox', 'product_category_name', 'product_name', 'photo'])
            ->addIndexColumn();
    }
    ...
    {
        return $model->newQuery()->with(['category', 'productItemImages']);
    }
    ...
    public function html(): HtmlBuilder
    {
        return $this->builder()
                    ...
                    ->minifiedAjax(route('products.items.index', [], true))
                    //->dom('Bfrtip')
                    ->orderBy(1)
                    ->selectStyleSingle()
                    ->buttons([
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('product_name')->orderable(true)->searchable(true),
