...
class LanguageDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('updated_by', function ($row) {
                ...
                $query->join('users as updater', 'master_languages.updated_by', '=', 'updater.id')
                    ->select(['master_languages.*', 'updater.name as updater_name'])
    ...
    public function html(): HtmlBuilder
    {
        return $this->builder()
                    ...
                    ->minifiedAjax(route('master.languages.index', [], true))
                    //->dom('Bfrtip')
                    ->orderBy(1)
                    ->selectStyleSingle()
                    ->buttons([
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('language_name')->orderable(true)->searchable(true),
            Column::make('language_code')->orderable(true)->searchable(true),
