...
class VenueRoomDataTable extends DataTable
{
    use DataTableCustom;
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->orderColumn('venue_name', function ($query, $order) {
                $query->join('master_venues', 'master_venue_rooms.venue_id', '=', 'master_venues.id')
                    ->select(['master_venue_rooms.*', 'master_venues.venue_name'])
                    ->orderBy('master_venues.venue_name', $order);
            })

            ->addColumn('photo', function ($row) {
                $images = $row->venueRoomImages;
                if ($images->isEmpty()) {
                        ...
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="' . $photoId . '_label">' . 'Room Image' . '</h5>
            ...
            ->addColumn('updated_by', function ($row) {
                ...
                $query->join('users as updater', 'master_venue_rooms.updated_by', '=', 'updater.id')
                    ->select(['master_venue_rooms.*', 'updater.name as updater_name'])
     ...
     */
    public function query(VenueRoom $model): QueryBuilder
    {
        return $model->newQuery()->with(['user', 'venue', 'language', 'venueRoomImages']);
    }
    ...
    {
        return $this->builder()
                    ->setTableId('venueroom-table')
                    ->columns($this->getColumns())
                    ->minifiedAjax(route('venues.rooms.index', [], true))
                    //->dom('Bfrtip')
                    ->orderBy(1)
                    ->selectStyleSingle()
                    ->buttons([
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('room_name')->orderable(true)->searchable(true),
