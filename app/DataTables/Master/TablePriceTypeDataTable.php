...
class TablePriceTypeDataTable extends DataTable
{
    use DataTableCustom;
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('updated_by', function ($row) {
                ...
                $query->join('users as updater', 'master_table_price_types.updated_by', '=', 'updater.id')
                    ->select(['master_table_price_types.*', 'updater.name as updater_name'])
     ...
     */
    public function query(TablePriceType $model): QueryBuilder
    {
    ...
    {
        return $this->builder()
                    ->setTableId('tablepricetype-table')
                    ->columns($this->getColumns())
                    ->minifiedAjax(route('tables.price.type.index', [], true))
                    //->dom('Bfrtip')
                    //->dom('Bfrtip')
                    ->orderBy(1)
                    ->selectStyleSingle()
    ...
    {
        return 'TablePriceType_' . date('YmdHis');
    }
}
