...
class HomepageDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->orderColumn('language_name', function ($query, $order) {
                $query->join('master_languages as lang', 'cms_homepage.lang_id', '=', 'lang.id')
                    ->select(['cms_homepage.*', 'lang.language_name as language_name'])
                    ->orderBy('language_name', $order);
            })

            ->addColumn('photo', function ($row) {
                $images = $row->homepageHeroImages;
                if ($images->isEmpty()) {
                    return '<span class="text-muted">No Image</span>';
                }

                $defaultImage = $images->where('is_default', 1)->first();
                $otherImages = $images->where('is_default', 0);

                // gabungin gambar, default image paling depan
                $sortedImages = $defaultImage ? collect([$defaultImage])->merge($otherImages) : $otherImages;
                ...
                foreach ($sortedImages as $index => $image) {
                    ...
                    <div class="carousel-item ' . $activeClass . '">
                        <img class="d-block img-fluid mx-auto" src="' . $imagePath . '" alt="Homepage Hero Image">
                    </div>';
                }
                        ...
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="' . $photoId . '_label">' . "Hero Section Image" . '</h5>
            ...
            ->addColumn('updated_by', function ($row) {
                ...
                $query->join('users as updater', 'cms_homepage.updated_by', '=', 'updater.id')
                    ->select(['cms_homepage.*', 'updater.name as updater_name'])
            ...
            ->rawColumns(['checkbox', 'language_name', 'photo'])
            ->addIndexColumn();
    }
    ...
    {
        return $model->newQuery()->with(['user', 'homepageHeroImages', 'language']);
    }
    ...
    public function html(): HtmlBuilder
    {
        return $this->builder()
                    ...
                    ->minifiedAjax(route('cms.homepage.index', [], true))
                    //->dom('Bfrtip')
                    ->orderBy(1)
                    ->selectStyleSingle()
                    ->buttons([
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::computed('hero_title')->title("Hero Title")->orderable(true)->searchable(true),
