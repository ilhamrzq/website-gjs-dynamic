...
class MenuDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->orderColumn('language_name', function ($query, $order) {
                $query->join('master_languages as lang', 'cms_menus.lang_id', '=', 'lang.id')
                    ->select(['cms_menus.*', 'lang.language_name as language_name'])
            ...
            ->addColumn('updated_by', function ($row) {
                ...
                $query->join('users as updater', 'cms_menus.updated_by', '=', 'updater.id')
                    ->select(['cms_menus.*', 'updater.name as updater_name'])
            ...
            ->rawColumns(['checkbox', 'language_name'])
            ->addIndexColumn();
    }
    ...
    public function html(): HtmlBuilder
    {
        return $this->builder()
                    ...
                    ->minifiedAjax(route('cms.menu.index', [], true))
                    //->dom('Bfrtip')
                    ->orderBy(1)
                    ->selectStyleSingle()
                    ->buttons([
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::computed('category_name')->title("Category Name")->orderable(true)->searchable(true),
            Column::computed('language_name')->title("Language")->orderable(true)->searchable(true),
            Column::computed('menu_name')->title("Menu Name")->orderable(true)->searchable(true),
            Column::computed('menu_path')->title("Menu Path")->orderable(true)->searchable(true),
            Column::computed('is_menu')->title("Active Menu")->orderable(true)->searchable(true),
