                ...
                <div class="card">
                    ...
                    <div class="card-header d-flex">
                        <ul class="flex-grow-1 nav nav-tabs-custom rounded card-header-tabs border-bottom-0"
                            role="tablist">
                            <li class="nav-item">
                                <a class="nav-link {{ session('activeTab', 'roomData') == 'roomData' ? 'active' : '' }}"
                                    data-bs-toggle="tab" href="#roomData" role="tab">
                                    @if (!request()->routeIs('venues.rooms.edit') && !request()->routeIs('venues.rooms.show'))
                                        {{ __('Venue Room') }}
                                    @endif
                            ...
                            <li class="nav-item">
                                <a class="nav-link {{ session('activeTab') == 'venueRoomImage' ? 'active' : '' }}"
                                    data-bs-toggle="tab" href="#venueRoomImage" role="tab"
                                    @if (!request()->routeIs('venues.rooms.edit') && !request()->routeIs('venues.rooms.show')) style="display: none;" @endif>
                                    {{ __('Images of the ' . Str::title($record->room_name ?? '')) }}
                                </a>
                            </li>
                        </ul>
                    </div>
                    ...
                    <div class="card-body">
                        <div class="tab-content">
                            ...
                            {{-- Edit Venue --}}
                            <div class="tab-pane nav-link {{ session('activeTab', 'roomData') == 'roomData' ? 'active' : '' }}"
                                id="roomData" role="tabpanel">
                                <div class="live-preview">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-6 col-sm-12">
                                            <div class="mb-3">
                                                <x-input-label for="room-name" :value="__('Room Name')" /><span
                                                    class="text-danger">*</span>
                                                <x-text-input type="text" name="room_name" :value="old('room_name', $record->room_name ?? '')"
                                                    class="form-control" :error="$errors->has('room_name')"
                                                    placeholder="Enter the venue room name" />
                                                <x-input-error class="mt-2" :messages="$errors->get('room_name')" />
                                            </div>
                                        </div>

                                        <div class="col-md-6 col-sm-12">
                                            <div class="mb-3">
                                                <x-input-label for="room-description" :value="__('Room Description')" /><span
                                                    class="text-danger">*</span>
                                                <x-text-input type="text" name="room_description" :value="old('room_description', $record->room_description ?? '')"
                                                    class="form-control" :error="$errors->has('room_description')"
                                                    placeholder="Enter the venue room description" />
                                                <x-input-error class="mt-2" :messages="$errors->get('room_description')" />
                                            </div>
                                        </div>

                                        <div class="col-md-6 col-sm-12 mb-3">
                                            <x-input-label for="room_minimum_charge" :value="__('Room Minimum Charge')" />
                                            <span class="text-danger">*</span>
                                            <div class="input-group">
                                                <span class="input-group-text">Rp</span>
                                                <x-text-input type="text" id="room_minimum_charge"
                                                    name="room_minimum_charge" :value="old(
                                                        'room_minimum_charge',
                                                        $record->room_minimum_charge ?? '',
                                                    )" data-format="currency"
                                                    class="form-control" :error="$errors->has('room_minimum_charge')" placeholder="10000" />
                                            </div>
                                            <x-input-error class="mt-2" :messages="$errors->get('room_minimum_charge')" />
                                        </div>
                            ...
                            {{-- Edit Venue Images --}}
                            <div class="tab-pane nav-link {{ session('activeTab') == 'venueRoomImage' ? 'active' : '' }}"
                                id="venueRoomImage" role="tabpanel">
                                ...
                                <div class="row">
                                    <div class="col-lg-12">
                                        <div class="card-header">
                                            <h5 class="card-title mb-0">{{ $room_name ?? '' }} Images</h5>
                                        </div>
                                        <div class="card-body">
                                                            ...
                                                            <td class="text-center">
                                                                <div class="dropdown d-inline-block">
                                                                    ...
                                                                    <ul class="dropdown-menu dropdown-menu-end">
                                                                        ...
                                                                        <form
                                                                            action="{{ route('venues.rooms.delete.image', $image->id) }}"
                                                                            method="post"
    ...
    @push('custom-script')
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                document.querySelectorAll('input[name="room_minimum_charge"]').forEach(function(input) {
        ...
        <script>
            $(document).ready(function() {
                ...
                $('.switch-default').on('change', function() {
                    ...
                    var updateUrl = "{{ route('venues.rooms.update.default.image', ':venueRoomImageId') }}";
                    var url = updateUrl.replace(':venueRoomImageId', imgId);
        ...
        <script>
            ...
            var myDropzone = new Dropzone("#image-dropzone", {
                url: "{{ route('venues.rooms.upload.images') }}", // URL untuk upload
                params: {
                    encrypted: encrypted
                },
                thumbnailWidth: 80,
