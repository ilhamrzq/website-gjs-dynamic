<x-app-layout>
                ...
                <div class="card">
                    ...
                    <div class="card-header d-flex">
                        <ul class="flex-grow-1 nav nav-tabs-custom rounded card-header-tabs border-bottom-0"
                            role="tablist">
                            <li class="nav-item">
                                <a class="nav-link {{ session('activeTab', 'eventData') == 'eventData' ? 'active' : '' }}"
                                    data-bs-toggle="tab" href="#eventData" role="tab">
                                    @if (!request()->routeIs('venues.events.edit') && !request()->routeIs('venues.events.show'))
                                        {{ __('Event') }}
                                    @endif
                            ...
                            <li class="nav-item">
                                <a class="nav-link {{ session('activeTab') == 'eventImages' ? 'active' : '' }}"
                                    data-bs-toggle="tab" href="#eventImage" role="tab"
                                    @if (!request()->routeIs('venues.events.edit') && !request()->routeIs('venues.events.show')) style="display: none;" @endif>
                                    {{ __('Images of the ' . Str::title($record->event_title ?? '')) }}
                                </a>
                            </li>
                        </ul>
                    </div>
                    ...
                    <div class="card-body">
                        <div class="tab-content">
                            ...
                            {{-- Edit Event --}}
                            <div class="tab-pane nav-link {{ session('activeTab', 'eventData') == 'eventData' ? 'active' : '' }}"
                                id="eventData" role="tabpanel">
                                <div class="live-preview">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="row align-items-center">
                                            ...
                                            <div class="col-md-6 col-sm-12 mb-3">
                                                <x-input-label for="event-status" :value="__('Status')" /><span
                                                    class="text-danger">*</span>
                                                <select class="js-example-basic-single" name="event_status">
                                                    <option value="COMING_SOON"
                                                        @if (old('event_status', $record->event_status ?? '') == 'COMING_SOON') selected @endif>Coming Soon
                                                    </option>
                                                    <option value="NEWS"
                                                        @if (old('event_status', $record->event_status ?? '') == 'NEWS') selected @endif>News</option>
                                                </select>
                                            </div>

                                            <div class="col-md-6 col-sm-12">
                                                <div class="mb-3">
                                                    <x-input-label for="event-title" :value="__('Title')" /><span
                                                        class="text-danger">*</span>
                                                    <x-text-input type="text" name="event_title" :value="old('event_title', $record->event_title ?? '')"
                                                        class="form-control" :error="$errors->has('event_title')"
                                            ...
                                            <div class="col-md-6 col-sm-12">
                                                <div class="mb-3">
                                                    <x-input-label for="event-description" :value="__('Description')" /><span
                                                        class="text-danger">*</span>
                                                    <x-text-input type="text" name="event_description"
                                                        :value="old(
                                                            'event_description',
                                                            $record->event_description ?? '',
                                                        )" class="form-control" :error="$errors->has('event_description')"
                                            ...
                                            <div class="col-md-6 col-sm-12">
                                                <div class="mb-3">
                                                    <x-input-label for="event-start_date" :value="__('Start date')" /><span
                                                        class="text-danger">*</span>
                                                    <x-text-input type="date" name="event_start_date"
                                                        :value="old(
                                                            'event_start_date',
                                                            $record->event_start_date ?? '',
                                                        )" class="form-control" :error="$errors->has('event_start_date')"
                                                        placeholder="Enter the event start_date" />
                                                    <x-input-error class="mt-2" :messages="$errors->get('event_start_date')" />
                                                </div>
                                            </div>

                                            <div class="col-md-6 col-sm-12">
                                                <div class="mb-3">
                                                    <x-input-label for="event-end_date" :value="__('End date')" /><span
                                                        class="text-danger">*</span>
                                                    <x-text-input type="date" name="event_end_date"
                                                        :value="old('event_end_date', $record->event_end_date ?? '')" class="form-control" :error="$errors->has('event_end_date')"
                                                        placeholder="Enter the event end_date" />
                                                    <x-input-error class="mt-2" :messages="$errors->get('event_end_date')" />
                                                </div>
                                            </div>
                                        </div>

                                        <div class="d-flex flex-column gap-2">
                                            <label for="event-content" class="form-label fw-semibold">
                                                {{ __('Content') }} <span class="text-danger">*</span>
                                            </label>

                                            <!-- Hidden input -->
                                            <input type="hidden" name="event_content" id="event_content"
                                                value="{{ old('event_content', $record->event_content ?? '') }}">

                                            <!-- CKEditor replaces this textarea -->
                                            <textarea id="editor" name="event_content">
                                                    {{ old('event_content', $record->event_content ?? '') }}
                                                </textarea>
                                        </div>
                            ...
                            {{-- Edit Event Images --}}
                            <div class="tab-pane nav-link {{ session('activeTab') == 'eventImages' ? 'active' : '' }}"
                                id="eventImage" role="tabpanel">
                                ...
                                <div class="row">
                                    <div class="col-lg-12">
                                        <div class="card-header">
                                            <h5 class="card-title mb-0">{{ $event_title ?? '' }} Images</h5>
                                        </div>
                                        <div class="card-body">
                                            <table id="event-images"
                                                            ...
                                                            <td class="text-center">
                                                                <div class="dropdown d-inline-block">
                                                                    ...
                                                                    <ul class="dropdown-menu dropdown-menu-end">
                                                                        ...
                                                                        <form
                                                                            action="{{ route('venues.events.delete.image', $image->id) }}"
                                                                            method="post"
    ...
    @push('custom-script')
        ...
        <script>
            ClassicEditor
                .create(document.querySelector('#editor'), {
                    ckfinder: {
                        uploadUrl: "{{ route('venues.events.upload.editor.image') }}?_token={{ csrf_token() }}"
                    }
                })
                .then(editor => {
                    console.log('CKEditor initialized', editor);

                    // Sync content ke hidden input setiap kali berubah
                    editor.model.document.on('change:data', () => {
                        document.querySelector('#event_content').value = editor.getData();
                    });

                    // Log upload gambar
                    const fileRepository = editor.plugins.get('FileRepository');
                    fileRepository.on('uploadComplete', (event, {
                        loader
                    }) => {
                        loader.file.then(file => {
                            console.log('Gambar berhasil diupload:', file);
                        });
                    });
                })
                .catch(error => {
        ...
        <script>
            ...
            ClassicEditor
                .create( document.querySelector( '#editor' ), {
                    plugins: [ Essentials, Heading, Bold, Italic, Font, Paragraph, Table, TableToolbar, Image, ImageInsert, ImageUpload, ImageToolbar, ImageCaption, ImageStyle, ImageResize, LinkImage ],
                    toolbar: [
                            'undo', 'redo',
                            '|',
                            'heading',
                ...
                .then(editor => {
                    ...
                    // Optional: Auto update the hidden input whenever content changes
                    editor.model.document.on('change:data', () => {
                        document.querySelector('#event_content').value = editor.getData();
                    });
                })
                ...
                Handle form submit to ensure latest content is in the hidden input
                document.addEventListener('DOMContentLoaded', function () {
                    const form = document.querySelector('form');
                    if (form) {
                        form.addEventListener('submit', function () {
                            if (ckEditorInstance) {
                                document.querySelector('#event_content').value = ckEditorInstance.getData();
                            }
                        });
                    }
                });
        ...
        <script>
            $(document).ready(function() {
                ...
                $('.switch-default').on('change', function() {
                    ...
                    var updateUrl = "{{ route('venues.events.update.default.image', ':eventImageId') }}";
                    var url = updateUrl.replace(':eventImageId', imgId);
        ...
        <script>
            ...
            var myDropzone = new Dropzone("#image-dropzone", {
                url: "{{ route('venues.events.upload.images') }}", // URL untuk upload
                params: {
                    encrypted: encrypted
                },
                thumbnailWidth: 80,
