                ...
                <div class="card">
                    ...
                    <div class="card-header d-flex">
                        <ul class="flex-grow-1 nav nav-tabs-custom rounded card-header-tabs border-bottom-0"
                            role="tablist">
                            <li class="nav-item">
                                <a class="nav-link {{ session('activeTab', 'galleryData') == 'galleryData' ? 'active' : '' }}"
                                    data-bs-toggle="tab" href="#galleryData" role="tab">
                                    @if (!request()->routeIs('cms.sponsorships.edit') && !request()->routeIs('cms.sponsorships.show'))
                                        {{ __('Sponsorship Type') }}
                                    @endif

                                    @if (request()->routeIs('cms.sponsorships.edit') || request()->routeIs('cms.sponsorships.show'))
                                        {{ __('Edit Sponsorship Type') }}
                                    @endif
                                </a>
                            </li>

                            <li class="nav-item">
                                <a class="nav-link {{ session('activeTab') == 'sponsorshipImage' ? 'active' : '' }}"
                                    data-bs-toggle="tab" href="#sponsorshipImage" role="tab"
                                    @if (!request()->routeIs('cms.sponsorships.edit') && !request()->routeIs('cms.sponsorships.show')) style="display: none;" @endif>
                                    {{ __('Images') }}
                                </a>
                            </li>
                        </ul>
                    ...
                    <div class="card-body">
                        <div class="tab-content">
                            ...
                            {{-- Edit Homepage --}}
                            <div class="tab-pane nav-link {{ session('activeTab', 'galleryData') == 'galleryData' ? 'active' : '' }}"
                                id="galleryData" role="tabpanel">
                                    ...
                                    <div class="row">
                                        <div class="row align-items-center">
                                            <div class="col-md-6 col-sm-12">
                                                <div class="mb-3">
                                                    <x-input-label for="sponsor-type-name" :value="__('Sponsor Type Name')" /><span
                                                        class="text-danger">*</span>
                                                    <x-text-input type="text" name="sponsor_type_name"
                                                        :value="old(
                                                            'sponsor_type_name',
                                                            $record->sponsor_type_name ?? '',
                                                        )" class="form-control" :error="$errors->has('sponsor_type_name')"
                                                        placeholder="Enter the sponsor type name" />
                                                    <x-input-error class="mt-2" :messages="$errors->get('sponsor_type_name')" />
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div class="tab-pane nav-link {{ session('activeTab') == 'sponsorshipImage' ? 'active' : '' }}"
                                id="sponsorshipImage" role="tabpanel">
                                ...
                                <div class="row">
                                    <div class="col-lg-12">
                                        <div class="card-header">
                                            <h5 class="card-title mb-0">Galleries Images</h5>
                                        </div>
                                        <div class="card-body">
                                            <table id="building-images"
                                                class="table table-bordered dt-responsive table-striped align-middle"
                                                style="width: 100%">
                                                <thead>
                                                    <tr>
                                                        <th class="text-center">No</th>
                                                        {{-- <th class="text-center">Set Cover Image</th> --}}
                                                        <th>Name</th>
                                                        <th>Size</th>
                                                        <th>Updated At</th>
                                                ...
                                                <tbody>
                                                    @foreach ($sponsorship_images as $image)
                                                        <tr>
                                                            <td class="text-center"> {{ $loop->iteration }} </td>
                                                            {{-- <td>
                                                                <div class="form-check form-switch text-center form-switch-secondary">
                                                                    <input class="form-check-input switch-default" type="checkbox" role="switch" id="{{ $image->file_name . $image->id }}" 
                                                                    @if ($image->is_default == true) checked @endif data-id={{ $image->id }}>
                                                                </div>
                                                            </td> --}}
                                                            ...
                                                            <td class="text-center">
                                                                <div class="dropdown d-inline-block">
                                                                    ...
                                                                    <ul class="dropdown-menu dropdown-menu-end">
                                                                        ...
                                                                        <form
                                                                            action="{{ route('cms.sponsorships.delete.image', $image->id) }}"
                                                                            method="post"
    ...
    @push('custom-script')
        <script>
            ...
            $(document).ready(function() {
                ...
                $('.switch-default').on('change', function() {
                    ...
                    var updateUrl =
                        "{{ route('cms.sponsorships.update.default.image', ':sponsorshipImageId') }}";
                    var url = updateUrl.replace(':sponsorshipImageId', imgId);
        ...
        <script>
            ...
            var myDropzone = new Dropzone("#image-dropzone", {
                url: "{{ route('cms.sponsorships.upload.images') }}", // URL untuk upload
                params: {
                    encrypted: encrypted
                },
                thumbnailWidth: 80,
