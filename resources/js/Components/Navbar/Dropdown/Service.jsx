...
import data from '../../../data/Navbar/services.json';

export default function ServiceNavbarDropdown({ lang_code }) {
  const [isOpenService, setIsOpenService] = useState(false);

  const datas = lang_code === 'id' ? data.services_id : data.services_en;

  return (
    <div onMouseEnter={() => setIsOpenService(true)} onMouseLeave={() => setIsOpenService(false)} className="flex items-center">
          ...
          <div className="dropdown-content-about py-12 absolute z-20 w-full left-0 right-0 top-[110px] bg-neutral-white-100 mt-2">
            <div className="grid grid-cols-3 grid-flow-col w-10/12 mx-auto my-3 gap-72">
              ...
              <div className="col-span-1">
                <img alt="navbar-service" src="/images/services/navbar-service.webp" className="h-full rounded-2xl object-cover" />
              </div>

              {/* Right Content */}
              <div className="col-span-2 w-full self-center">
                <div className="flex flex-col">
                  <h1 className="title font-bold fs-40 color-primary-blue-100">{lang_code === 'id' ? 'Layanan' : 'Services'}</h1>
            ...
            <img src="/images/images/about-us.webp" className="absolute right-0 bottom-0" alt="about-us-image" />
          </div>
        )}
      </div>
    </div>
